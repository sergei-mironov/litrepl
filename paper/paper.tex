...
\begin{figure*}[hbt!]
  \centering
  \begin{verbatim}
  document ::= (code_section | result_section | ignore_section | text)*
  code_section ::= code_bmarker code_section_text code_emarker
  result_section ::= result_bmarker result_section_text result_emarker
  ignore_section ::= ignore_bmarker ignore_section_text ignore_emarker
  \end{verbatim}
  \caption{Example output in a verbatim format displayed above two columns.}
...
This parsing strategy allows Litrepl to navigate and process documents with
precision, ensuring that relevant sections are evaluated while non-relevant
parts are effectively ignored. By integrating document format-specific markers,
Litrepl maintains flexibility and adaptability in diverse document scenarios.
