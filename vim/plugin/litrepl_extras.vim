...
fun! LitReplAISelection() range
  ...
  let prompt = "".
        ...
        \ "/cp file:\"".expand('%')."\" buffer:file\n". " [3]
        \ input " [1]

  let [errcode, result] = LitReplRunV('eval-code ai', prompt)
  if errcode == 0
...
if !exists(":LAI")
  command! -bar -nargs=0 LAI call LitReplAISelection()
endif
