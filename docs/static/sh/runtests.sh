...
grep -q 'non-existent-ipython: not found' out1.md
grep -q 'non-existent-ipython: not found' err.txt
)} #}}}
...
header
litrepl-dummy.sh --selection-paste - --prompt input --output-format markdown --command eval-code
selection
END
footer
EOF
...
header
litrepl-dummy.sh --selection-paste - --output-format markdown --command eval-code
selection
END
footer
EOF
...
header
selection
litrepl-dummy.sh --prompt input --output-format markdown --command eval-code
END
footer
EOF
